...
#define _MAX_USB_USER_ELEM (4096)
...
void ioPushInUserUsb(uint8_t *pw, int *len)
{
  uint8_t *pr = pw;
  for (int i=0; i < *len; i++) {
    *_usb_p_write++ = *pr++;
...
bool ioRawWriteBuffer(uint8_t *buff, int count)
{
  ...
  while(1){
    ...
    status = ux_device_class_cdc_acm_write_run(cdc_acm, (unsigned char *)buff, count,
                                     &actual_length);
...
#else
  HAL_StatusTypeDef status = HAL_OK;
  uint32_t m_count = (uint32_t)count;
  while ((_usb_nb_w_item - _usb_nb_r_item) < m_count) {};
...
static const char *devIdToStr(uint16_t dev_id)
{
...
#elif defined(STM32H7P)
  case 0x47B: str = "STM32H7Pxx"; break;
  ...
  case 0x185: str = "Corstone SSE-310 FVP"; break;  /* */
#elif defined(CORSTONE_315)
  case 0x186: str = "Corstone SSE-315 FVP"; break;  /* */
#elif defined(CORSTONE_320)
  case 0x187: str = "Corstone SSE-320 FVP"; break;  /* */
#elif defined(SR5E1) /* STELLAR-E device */
  case 0x2511: str = "SR5E1"; break;  /* see RM0483 */
#elif defined(SR6X) /* STELLAR P & G devices */
  case 0x2643: str = "SR6P3"; break;  /* see RM0509 */
  case 0x2646: str = "SR6P6"; break;  /* see RM0496 */
  case 0x2647: str = "SR6P7"; break;  /* see RM0505 */
  case 0x2633: str = "SR6G3"; break;  /* see RM0538 */
  case 0x2636: str = "SR6G6"; break;  /* see RM0518 */
  case 0x2637: str = "SR6G7"; break;  /* see RM0500 */
  case 0x2A47: str = "SR6P7G7"; break;  /* see RM0482 */
  case 0x2663: str = "SR6P3E"; break;  /* see RM0544 */
#elif defined(STM32V8)
  case 0x499: str = "STM32V8"; break;  /* */
#endif
  default:    str = "UNKNOWN";
  }
  return str;
...
uint32_t getFlashCacheConf(void)
{
  ...
  __I uint32_t acr = FLASH->ACCR;
  mconf |= ((acr & FLASH_ACCR_LATENCY_Msk) >> FLASH_ACCR_LATENCY_Pos);
...
#if defined(ARMCM85) || defined(STM32V8)
  mconf |= MCONF_V8;
  __I uint32_t ccr = SCB->CCR;
  if (ccr & SCB_CCR_DC_Msk)
    mconf |= MCONF_CORE_DCACHE;
